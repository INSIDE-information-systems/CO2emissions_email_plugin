...
var gettingCurrent = browser.tabs.query({currentWindow: true});
gettingCurrent.then(async tabInfo => {
    ...
    let headerSize = HEADER_SIZE + lengthInUtf8Bytes(to.join(",") + cc.join(",") + bcc.join(","));
    ...
    document.getElementById("recipents-count").innerText = recipentsCount;

    let totalSize = headerSize + messageBodySize + attachmentsSize;
    let sizeTimesRecipents = totalSize * recipentsCount;
